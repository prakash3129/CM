    ...
    public class EAF
    {
        ...
        public DataTable Validation(List<DataTable> lstDatable)
        {
            ...
            try
            {
                ...
                {
                    string sTRContactstatusTobeValidated = dtProjectInfo.Select("Key = '" + sAccessType + "ContactStatusToBeValidated' AND Type = 'String'")[0]["Value"].ToString();
                    if (sTRContactstatusTobeValidated.Length > 0)
                    {
                        if (dtContact.Select(sAccessType + "_CONTACT_STATUS IN (" + sTRContactstatusTobeValidated + ") AND DEPARTMENT='Advertising'").Length > 2)
                        {
                            dtMessage.Rows[0][1] += "Maximum number of contacts for Advertising is 2" + Environment.NewLine;
                        }

                        if (dtContact.Select(sAccessType + "_CONTACT_STATUS IN (" + sTRContactstatusTobeValidated + ") AND DEPARTMENT='Data Sales'").Length > 2)
                        {
                            dtMessage.Rows[0][1] += "Maximum number of contacts for Data Sales is 2" + Environment.NewLine;
                        }
