    ...
    public class EAF
    {
        ...
        public DataTable GetList(List<DataTable> lstDatable)
        {
            ...
            try
            {
                ...
                dtJobTitle.Rows.Add("CEO / President / Vice President", "3");
                dtJobTitle.Rows.Add("General Manager / Managing Director", "3");
                dtJobTitle.Rows.Add("Commercial Manager", "3");
                dtJobTitle.Rows.Add("Business Manager", "3");
                dtJobTitle.Rows.Add("Base Oils Trader", "3");
                dtJobTitle.Rows.Add("Marketing Manager", "3");
                ...
                if (drrCount.Length > 0)
                {
                    ...
                    {
                        int x = dtContactCount.Select("DEPARTMENT = '" + dr[0].ToString() + "'").Length;
        ...
        public DataTable Validation(List<DataTable> lstDatable)
        {
            ...
            try
            {
                ...
                DataTable dtContactTable = Table(lstDatable, "MasterContact");


                if (dtContactTable.Rows.Count > 0)
                {
                    string sAccess = Table(lstDatable, "ProjectInfo").Select("key = 'AccessTo'")[0][1].ToString();
                    string TRContactstatusTobeValidated = Table(lstDatable, "ProjectInfo").Select("key = 'TRContactstatusTobeValidated' AND Type='String'")[0][1].ToString();
                    DataRow[] drrCount = dtContactTable.Select(sAccess + "_CONTACT_STATUS IN (" + TRContactstatusTobeValidated + ")");

                    if (drrCount.Length > 0)
                    {
                        List<string> lstJobFun = new List<string>();
                        lstJobFun.Add("CEO / President / Vice President");
                        lstJobFun.Add("General Manager / Managing Director");
                        lstJobFun.Add("Commercial Manager");
                        lstJobFun.Add("Business Manager");
                        lstJobFun.Add("Base Oils Trader");
                        lstJobFun.Add("Marketing Manager");

                        DataTable dtContactJobTitle = drrCount.CopyToDataTable();
                        foreach (string sJob in lstJobFun)
                        {
                            if(dtContactJobTitle.Select("DEPARTMENT = '"+sJob+"'").Length > 3)
                                dtMessage.Rows[0][1] += "Job function " + sJob + " cannot be more then 3 times"+Environment.NewLine;
                        }
