...
namespace GCC
{
    public partial class frmJobTitle_SelectionList : Office2007Form
    {
        public frmJobTitle_SelectionList()
        {
            InitializeComponent();
        ...
        Background bAccept = new Background(Color.LightCyan);
        Background bPending = new Background(Color.Gainsboro);
        Background bReject = new Background(Color.MistyRose);
        ...
        void Search(DataTable dtData)
        {
            ...
            for (int i = 0; i < dtData.Rows.Count; i++)
            {
                if (sHeader == dtData.Rows[i][sHeaderColumn].ToString().ToUpper())
                {
                    ...
                    if(sHeader.ToUpper() == "ACCEPTED")
                        GRSubRow.CellStyles.Default.Background = bAccept;
                    else if (sHeader.ToUpper() == "PENDING")
                        GRSubRow.CellStyles.Default.Background = bPending;
                    else
                        GRSubRow.CellStyles.Default.Background = bReject;
        ...
        private void txtSearch_TextChanged(object sender, EventArgs e)
        {
            ...
            string sStartsWith = string.Empty;
            if (switchStartsWith.Value)
                sStartsWith = "%";

            DataRow[] drrItem = dtItems.Select(sValueColumn + " LIKE '" + sStartsWith + txtSearch.Text.Trim().Replace("'", "''").Replace("(*)", "").Replace("[", string.Empty).Replace("]", string.Empty).Replace("%", string.Empty).Replace("[", string.Empty).Replace("]", string.Empty).Replace("%", string.Empty) + "%'", sHeaderColumn + " ASC");
            if (drrItem.Length > 0)
        ...
        protected override bool ProcessCmdKey(ref Message msg, Keys keyData)
        {
            ...
            if (keyData == (Keys.Control | Keys.T))            
                switchStartsWith.Value = !switchStartsWith.Value;
